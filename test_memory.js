...
test('setting and loading values from memory', function(t) {
  ...
  stack.write(255, 0, 1, memory.unsigned);
  stack.write(255, 1, 1, memory.unsigned);
  stack.write(-1, 2, 1, memory.signed);
  stack.write(1.25, 4, 4, memory.float);
  
  t.equal(stack.read(0,1, memory.unsigned), 255);
  t.equal(stack.read(1,1, memory.signed), -1);
  t.equal(stack.read(2,1, memory.unsigned), 255);
