...
test('setting and loading values from memory', function(t) {
  ...
  var stack = new memory.Memory(8);
  
  stack = stack.write(255, 1, 1, memory.unsigned);
  stack = stack.write(255, 2, 1, memory.unsigned);
  stack = stack.write(-1, 3, 1, memory.signed);
  stack = stack.write(1.25, 4, 4, memory.float);
  
  t.equal(stack.read(1,1, memory.unsigned), 255, 'simple read');
  t.equal(stack.read(2,1, memory.signed), -1, 'reading a different type');
  t.equal(stack.read(3,1, memory.unsigned), 255, 'reading a different type');
  t.equal(stack.read(4,4, memory.float), 1.25, 'reading a float');
  t.equal(stack.read(3,1, memory.signed), -1, 'simple read');
});
